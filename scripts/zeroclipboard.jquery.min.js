...
$(this.pairs[id]).data("zeroclipboard_ready",true);ZeroClipboard.update(id);break;case'mouseover':$(this.pairs[id]).trigger('mouseover');break;case'mouseout':$(this.pairs[id]).trigger('mouseout');$(this.pairs[id]).trigger('mouseup');break;case'mouseup':$(this.pairs[id]).trigger('mouseup');break;case'mousedown':$(this.pairs[id]).trigger('mousedown');break;}},update:function(id){var original=this.pairs[id];if($(original).data("zeroclipboard_ready")){var flash=document.getElementById('zeroclipboard_swf_'+id);if($(original).data('zeroclipboard_resize')){var elemWidth=$(original).outerWidth();var elemHeight=$(original).outerHeight();var elemPos=$(original).offset();$(flash).attr('width',elemWidth).attr('height',elemHeight);$(flash).parent().css({'width':elemWidth+'px','height':elemHeight+'px','top':elemPos.top,'left':elemPos.left});$(original).data('zeroclipboard_resize',false);}
